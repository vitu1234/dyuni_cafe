...
//print_r($_POST);
  //add post
  if(isset($_POST['title']) && isset($_POST['img']) && isset($_POST['content'])  && !empty($_POST['title']) && !empty($_POST['img']) && !empty($_POST['content']) ){
    ...
    if($operation->countAll("SELECT *FROM news WHERE title = '$title'") == 0){
      //upload photo
    $data = $_POST['img'];
	...
	$data_="n_image='".$filename."'";
        ...
        if($operation->insertData($table,$data)==1){
            //get the recent id
            $id = $operation->retrieveSingle("SELECT *FROM news ORDER BY news_id DESC");
          echo json_encode(array("code"=>1,"msg"=>"News article saved, please wait!","id"=>$id['news_id']));
        }else{
          echo json_encode(array("code"=>2,"msg"=>"An error occured while saving news article!"));
        }
        
      
    } else{
      echo json_encode(array("code"=>2,"msg"=>"Post with same title already exists try another title!"));
    } 
